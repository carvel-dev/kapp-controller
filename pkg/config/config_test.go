...
func Test_ShouldSkipTLSForDomain(t *testing.T) {
	configMap := &v1.ConfigMap{
		...
		Data: map[string]string{
			"dangerousSkipTLSVerify": "always.trustworthy.com, also.trusted.net:123456",
		},
	}
	k8scs := k8sfake.NewSimpleClientset(configMap)
	config, err := kcconfig.GetConfig(k8scs)
	...
	assert.True(t, config.ShouldSkipTLSForDomain("also.trusted.net"))
	assert.True(t, config.ShouldSkipTLSForDomain("also.trusted.net:123456"))
}
