...
func (f *AppFactory) NewCRDPackageRepo(app *kcv1alpha1.App, pkgr *pkgv1alpha1.PackageRepository, log logr.Logger) *CRDApp {
	fetchFactory := fetch.NewFactory(f.ClusterClient, fetch.VendirOpts{SkipTLSConfig: f.KcConfig}, f.CmdRunner, f.ControllerVersion)
	templateFactory := template.NewFactory(f.ClusterClient, fetchFactory, false, f.CmdRunner)
	deployFactory := deploy.NewFactory(f.ClusterClient, nil, f.CmdRunner)
