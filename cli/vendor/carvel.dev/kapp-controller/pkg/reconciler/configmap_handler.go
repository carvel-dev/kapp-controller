...
func (sch *ConfigMapHandler) Create(_ context.Context, evt event.TypedCreateEvent[*corev1.ConfigMap], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	sch.enqueueAppsForUpdate(evt.Object.GetName(), evt.Object.GetNamespace(), q)
}

// Update is called in response to an update event
func (sch *ConfigMapHandler) Update(_ context.Context, evt event.TypedUpdateEvent[*corev1.ConfigMap], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	sch.enqueueAppsForUpdate(evt.ObjectNew.GetName(), evt.ObjectNew.GetNamespace(), q)
}

// Delete is called in response to a delete event
func (sch *ConfigMapHandler) Delete(_ context.Context, evt event.TypedDeleteEvent[*corev1.ConfigMap], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	sch.enqueueAppsForUpdate(evt.Object.GetName(), evt.Object.GetNamespace(), q)
...
func (sch *ConfigMapHandler) Generic(_ context.Context, _ event.TypedGenericEvent[*corev1.ConfigMap], _ workqueue.TypedRateLimitingInterface[reconcile.Request]) {
}

func (sch *ConfigMapHandler) enqueueAppsForUpdate(cfgmName, cfgmNamespace string, q workqueue.TypedRateLimitingInterface[reconcile.Request]) error {
